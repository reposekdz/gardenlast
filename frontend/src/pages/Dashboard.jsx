...
const GenderChart = ({ male, female, t }) => {
        ...
        <div className="space-y-3">
            <div className="flex items-center justify-between text-sm">
                <span className="flex items-center gap-2">
                    ...
                    <span className="text-gray-600">{t('dash_main.sections.male')}</span>
                </span>
            ...
            <div className="flex items-center justify-between text-sm">
                <span className="flex items-center gap-2">
                    ...
                    <span className="text-gray-600">{t('dash_main.sections.female')}</span>
                </span>
...
const CollectionGauge = ({ rate, t }) => {
    ...
    return (
        <div className="relative pt-6">
            ...
            <p className="text-center text-xs text-gray-400">{t('dash_main.sections.collection_rate_caption')}</p>
        </div>
    );
};
...
const Dashboard = () => {
    ...
    const handleRefresh = async () => {
        ...
        await fetchData();
        setRefreshing(false);
        toast.success(t('dash_main.refresh_success'));
    };
    ...
    const roleConfig = {
        admin: { titleKey: 'admin', color: 'from-purple-600 to-purple-800', roleColor: 'bg-purple-500' },
        dod: { titleKey: 'dod', color: 'from-red-600 to-red-800', roleColor: 'bg-red-500' },
        director_of_discipline: { titleKey: 'dod', color: 'from-red-600 to-red-800', roleColor: 'bg-red-500' },
        accountant: { titleKey: 'accountant', color: 'from-green-600 to-green-800', roleColor: 'bg-green-500' },
        stock_manager: { titleKey: 'stock_manager', color: 'from-amber-600 to-amber-800', roleColor: 'bg-amber-500' },
        parent: { titleKey: 'parent', color: 'from-pink-600 to-pink-800', roleColor: 'bg-pink-500' },
    };
    ...
    const allCards = [
        {
            icon: Users,
            label: t('dash_main.cards.total_students'),
            value: stats?.total_students,
            color: 'bg-primary-500',
            to: '/students',
            show: isAdmin || isDod || isAccountant,
            subValue: t('dash_main.sub_values.plus_this_month', { count: stats?.recent_students || 0 })
        },
        {
            icon: ShieldAlert,
            label: t('dash_main.cards.active_cases'),
            value: stats?.active_discipline,
            color: 'bg-red-500',
            to: '/discipline',
            show: isAdmin || isDod,
            subValue: t('dash_main.sub_values.resolved_this_month', { count: stats?.resolved_discipline || 0 })
        },
        {
            icon: DollarSign,
            label: t('dash_main.cards.total_collected'),
            value: stats?.total_fees_collected,
            color: 'bg-green-500',
            to: '/finance',
            show: isAdmin || isAccountant || isDod,
            trend: 'up',
            trendValue: t('dash_main.sub_values.collection_rate_label', { rate: stats?.collection_rate || 0 })
        },
        {
            icon: Package,
            label: t('dash_main.cards.low_stock'),
            value: stats?.low_stock,
            color: 'bg-amber-500',
            to: '/stock',
            show: isAdmin || isStock,
            subValue: t('dash_main.sub_values.items_need_attention')
        },
        {
            icon: Heart,
            label: t('dash_main.cards.parents'),
            value: stats?.total_parents,
            color: 'bg-pink-500',
            to: '/link-manager',
            show: isAdmin || isDod || isAccountant,
            subValue: t('dash_main.sub_values.registered')
        },
        {
            icon: Clock,
            label: t('dash_main.cards.pending_links'),
            value: stats?.pending_links,
            color: 'bg-orange-500',
            to: '/link-manager',
            show: isAdmin || isDod || isAccountant,
            subValue: t('dash_main.sub_values.requests_waiting')
        },
        {
            icon: Newspaper,
            label: t('dash_main.cards.applications'),
            value: stats?.pending_applications,
            color: 'bg-indigo-500',
            to: '/applications',
            show: isAdmin || isDod,
            subValue: t('dash_main.sub_values.pending_review')
        },
        {
            icon: UserCheck,
            label: t('dash_main.cards.on_leave'),
            value: stats?.on_leave,
            color: 'bg-blue-500',
            to: '/students',
            show: isAdmin || isDod,
            subValue: t('dash_main.sub_values.students')
        },
        {
            icon: MessageCircle,
            label: t('dash_main.cards.sms_balance'),
            value: smsBalance?.balance || t('common_extra.not_available'),
            color: 'bg-cyan-500',
            show: isAdmin,
            subValue: smsBalance?.message || t('dash_main.sub_values.airtime')
        },
    ];
        ...
        <div className="space-y-8 animate-fade-in">
            <div className={`bg-gradient-to-br ${currentRole.color} rounded-2xl px-8 py-8 text-white relative overflow-hidden shadow-2xl`}>
                ...
                <button
                    ...
                    title={t('dash_main.refresh_tooltip')}
                >
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <p className="text-white/80 text-sm font-medium">{t(`dash_main.role_titles.${currentRole.titleKey}`)}</p>
                    </div>
                    <h2 className="text-3xl font-black mb-2">{t('dash_main.welcome_user', { name: user.first_name || user.username })}</h2>
                    <p className="text-white/80 max-w-xl">{t('dash_main.tagline')}</p>
                    ...
                    {(isAdmin || isDod) && stats?.pending_links > 0 && (
                        <Link to="/link-manager" className="inline-flex items-center gap-2 mt-6 bg-accent-500 hover:bg-accent-400 text-primary-900 font-black px-5 py-3 rounded-xl text-sm transition-all shadow-lg hover:shadow-xl">
                            <Bell size={16} /> {t('dash_main.pending_link_requests', { count: stats.pending_links })}
                        </Link>
                    )}
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="font-bold text-gray-900 text-lg flex items-center gap-2">
                                <BarChart3 size={20} className="text-primary-600" /> {t('dash_main.sections.revenue_trend')}
                            </h3>
                            <span className="text-xs text-gray-400">{t('dash_main.sections.last_6_months')}</span>
                        </div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                        <h3 className="font-bold text-gray-900 text-lg mb-6 flex items-center gap-2">
                            <PieChart size={20} className="text-primary-600" /> {t('dash_main.sections.gender_distribution')}
                        </h3>
                        <GenderChart male={stats?.male_students || 0} female={stats?.female_students || 0} t={t} />
                    </div>
                </div>
            )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                        <h3 className="font-bold text-gray-900 text-lg mb-4 flex items-center gap-2">
                            <Activity size={20} className="text-green-600" /> {t('dash_main.sections.collection_rate')}
                        </h3>
                        <CollectionGauge rate={stats?.collection_rate || 0} t={t} />
                    </div>

                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                        <h3 className="font-bold text-gray-900 text-lg mb-4 flex items-center gap-2">
                            <Calendar size={20} className="text-amber-600" /> {t('dash_main.sections.todays_activity')}
                        </h3>
                        <div className="space-y-4">
                            <div className="flex items-center justify-between p-3 bg-green-50 rounded-xl">
                                <div className="flex items-center gap-3">
                                    <DollarSign size={20} className="text-green-600" />
                                    <span className="text-sm text-gray-600">{t('dash_main.sections.payments_label')}</span>
                                </div>
                                <span className="font-black text-green-600">{Number(stats?.today_payments || 0).toLocaleString()} {t('common_extra.currency_rwf')}</span>
                            </div>
                            <div className="flex items-center justify-between p-3 bg-blue-50 rounded-xl">
                                <div className="flex items-center gap-3">
                                    <Receipt size={20} className="text-blue-600" />
                                    <span className="text-sm text-gray-600">{t('dash_main.sections.transactions_label')}</span>
                                </div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                        <h3 className="font-bold text-gray-900 text-lg mb-4 flex items-center gap-2">
                            <BookOpen size={20} className="text-indigo-600" /> {t('dash_main.sections.top_trades')}
                        </h3>
                        <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <h3 className="font-bold text-gray-900 text-lg mb-5 flex items-center gap-2">
                        <Calendar size={20} className="text-primary-600" /> {t('dash_main.sections.quick_actions')}
                    </h3>
                    <div className="space-y-3">
                        {isAdmin && (
                            <>
                                <Link to="/students" className="flex items-center gap-3 p-4 rounded-xl bg-primary-50 hover:bg-primary-100 text-primary-700 transition-all group">
                                    ...
                                    <span className="text-sm font-bold">{t('dash_main.quick.add_student')}</span>
                                </Link>
                                <Link to="/link-manager" className="flex items-center gap-3 p-4 rounded-xl bg-pink-50 hover:bg-pink-100 text-pink-700 transition-all group">
                                    ...
                                    <span className="text-sm font-bold">{t('dash_main.quick.review_parent_requests')}</span>
                                </Link>
                                <Link to="/staff" className="flex items-center gap-3 p-4 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 transition-all group">
                                    ...
                                    <span className="text-sm font-bold">{t('dash_main.quick.add_staff')}</span>
                                </Link>
                                <Link to="/cms" className="flex items-center gap-3 p-4 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-700 transition-all group">
                                    ...
                                    <span className="text-sm font-bold">{t('dash_main.quick.post_news')}</span>
                                </Link>
                            </>
                        )}
                        {isDod && (
                            <>
                                <Link to="/students" className="flex items-center gap-3 p-4 rounded-xl bg-primary-50 hover:bg-primary-100 text-primary-700 transition-all group">
                                    <Users size={20} className="text-primary-600 group-hover:scale-110 transition-transform" />
                                    <span className="text-sm font-bold">{t('dash_main.quick.manage_students')}</span>
                                </Link>
                                <Link to="/discipline" className="flex items-center gap-3 p-4 rounded-xl bg-red-50 hover:bg-red-100 text-red-700 transition-all group">
                                    ...
                                    <span className="text-sm font-bold">{t('dash_main.quick.manage_discipline')}</span>
                                </Link>
                                <Link to="/applications" className="flex items-center gap-3 p-4 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 transition-all group">
                                    ...
                                    <span className="text-sm font-bold">{t('dash_main.quick.review_applications')}</span>
                                </Link>
                                <Link to="/link-manager" className="flex items-center gap-3 p-4 rounded-xl bg-pink-50 hover:bg-pink-100 text-pink-700 transition-all group">
                                    <Heart size={20} className="text-pink-500 group-hover:scale-110 transition-transform" />
                                    <span className="text-sm font-bold">{t('dash_main.quick.parent_links')}</span>
                                </Link>
                            </>
                        )}
                        {isAccountant && (
                            <>
                                <Link to="/finance" className="flex items-center gap-3 p-4 rounded-xl bg-green-50 hover:bg-green-100 text-green-700 transition-all group">
                                    ...
                                    <span className="text-sm font-bold">{t('dash_main.quick.manage_finances')}</span>
                                </Link>
                                <Link to="/link-manager" className="flex items-center gap-3 p-4 rounded-xl bg-pink-50 hover:bg-pink-100 text-pink-700 transition-all group">
                                    <Heart size={20} className="text-pink-500 group-hover:scale-110 transition-transform" />
                                    <span className="text-sm font-bold">{t('dash_main.quick.parent_links')}</span>
                                ...
                                <Link to="/students" className="flex items-center gap-3 p-4 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-700 transition-all group">
                                    ...
                                    <span className="text-sm font-bold">{t('dash_main.quick.view_students')}</span>
                                </Link>
                                <Link to="/applications" className="flex items-center gap-3 p-4 rounded-xl bg-purple-50 hover:bg-purple-100 text-purple-700 transition-all group">
                                    ...
                                    <span className="text-sm font-bold">{t('dash_main.quick.applications')}</span>
                                </Link>
                                <Link to="/stock" className="flex items-center gap-3 p-4 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-700 transition-all group">
                                    ...
                                    <span className="text-sm font-bold">{t('dash_main.quick.stock')}</span>
                                </Link>
                                <Link to="/parents" className="flex items-center gap-3 p-4 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 transition-all group">
                                    ...
                                    <span className="text-sm font-bold">{t('dash_main.quick.parents')}</span>
                                </Link>
                            </>
                        )}
                        {isStock && (
                            <Link to="/stock" className="flex items-center gap-3 p-4 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-700 transition-all group">
                                <Package size={20} className="text-amber-500 group-hover:scale-110 transition-transform" />
                                <span className="text-sm font-bold">{t('dash_main.quick.manage_inventory')}</span>
                            </Link>
                        )}
                        {isParent && (
                            <Link to="/parents" className="flex items-center gap-3 p-4 rounded-xl bg-pink-50 hover:bg-pink-100 text-pink-700 transition-all group">
                                <Heart size={20} className="text-pink-500 group-hover:scale-110 transition-transform" />
                                <span className="text-sm font-bold">{t('dash_main.quick.view_my_children')}</span>
                            </Link>
                        )}
                    </div>
                </div>
                ...
                <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <h3 className="font-bold text-gray-900 text-lg mb-5 flex items-center gap-2">
                        <TrendingUp size={20} className="text-primary-600" /> {t('dash_main.sections.recent_payments')}
                    </h3>
                    <div className="space-y-3">
                        {recentActivity.length > 0 ? (
                                ...
                                <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500">{activity.term || t('dash_main.sections.payments_label')} • {activity.receipt_number || t('common_extra.not_available')}</p>
                                        </div>
                                    </div>
                            ...
                            <div className="text-center py-8 text-gray-500">
                                ...
                                <p className="text-sm">{t('dash_main.sections.no_recent_payments')}</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="bg-gradient-to-r from-gray-100 to-gray-200 rounded-2xl p-6 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-gray-600 font-medium">{t('common_extra.system_status')}: {t('common_extra.operational')}</span>
                </div>
                <div className="text-sm text-gray-500">
                    {t('common_extra.system_version')}
                </div>
            </div>
        </div>
    );
